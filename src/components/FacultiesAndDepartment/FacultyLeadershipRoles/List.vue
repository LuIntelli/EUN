...
import {
  listFacultyLeadershipRoles,
  deleteFacultyLeadershipRoles,
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listFacultyLeadershipRoles(); // from your services
...
const handleDelete = async (id) => {
  try {
    await deleteFacultyLeadershipRoles(id);
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () =>
            router.push(`/faculty-leadership-roles/edit/${row.id}`),
        },
        row.id
      );
    },
