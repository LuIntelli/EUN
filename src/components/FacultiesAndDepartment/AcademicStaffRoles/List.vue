...
import {
  listAcademicStaffRoles,
  deleteAcademicStaffRoles,
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listAcademicStaffRoles(); // from your services
...
const handleDelete = async (id) => {
  try {
    await deleteAcademicStaffRoles(id);
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/academic/staff-roles/${row.id}`),
        },
        row.id
      );
    },
