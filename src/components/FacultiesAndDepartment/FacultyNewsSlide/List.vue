...
import {
  listFacultyNewSlides,
  deleteFacultyNewSlides,
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listFacultyNewSlides(); // from your services
...
const handleDelete = async (id) => {
  try {
    await deleteFacultyNewSlides(id);
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/faculty-news-slides/edit/${row.id}`),
        },
        row.id
      );
    },
