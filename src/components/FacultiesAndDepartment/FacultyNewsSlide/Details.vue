  ...
  <div class="p-6 w-full mx-auto">
    ...
    >
      <n-form-item label="Faculty News" path="faculty_news">
        <n-input v-model:value="formData.faculty_news" placeholder="2" />
      </n-form-item>
      <n-form-item label="Faculty News Title" path="faculty_news_title">
        <n-input
          v-model:value="formData.faculty_news_title"
          placeholder="Faculty of Medical Sciences News & Updates"
        />
      </n-form-item>
...
import {
  editFacultyNewSlides,
  facultyNewsSlideDetails,
...
const fetchData = async () => {
  try {
    ...
    const data = await facultyNewsSlideDetails(id);
    Object.assign(formData, data.data);
    photoPreview.value = formData["background_image"];
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (photoFile.value) {
      formData["background_image"] = photoFile.value;
    }

    console.log("Start submitting...", formData);
    await editFacultyNewSlides(route.params.id, formData);
    message.success("Updated successfully");
    router.push("/faculty-news-slides");
  } catch (err) {
    console.error(err);
