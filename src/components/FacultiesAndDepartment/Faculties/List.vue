<template>
  <div class="p-6">
    <h2 class="text-2xl font-bold text-[rgba(61,58,121,1)] mb-4">Faculties</h2>
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listFaculties(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/faculties/edit/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Department Count", key: "department_count" },
