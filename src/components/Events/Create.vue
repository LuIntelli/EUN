  ...
  <div class="p-6 w-full mx-auto">
    ...
    >
      ...
      <n-form-item label="Created By" path="created_by">
        <n-input v-model:value="formData.created_by" placeholder="3" />
      </n-form-item>
...
import { createEvent, listEventCategory } from "@/services/Event";
...
onMounted(async () => {
  ...
  try {
    const res = await listEventCategory();
    console.log(res.results);

    eventCategoryOptions.value = res.results.map((category) => ({
      label: category.name || `Category ID ${category.id}`,
      value: Number(category.id),
      faculty: category.category_name,
    }));
  } catch (err) {
...
watch(
  () => formData.category,
  (newVal, oldVal) => {
    const selected = eventCategoryOptions.value.find(
      (position) => position.value === newVal
    );
    if (selected) formData.category_name = selected?.label;
  }
);
  ...
  registration_required: [
    {
      required: false,
      type: "boolean",
      message: "Is Registration required is required",
      trigger: "change",
    },
  ],
  is_published: [
