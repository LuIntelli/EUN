...
import { listEventCategory, deleteEventCategory } from "@/services/Event";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listEventCategory(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/event/category/edit/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Event Count", key: "event_count" },
