...
import { listFAQS, deleteFAQS } from "@/services/Pages";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listFAQS(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/page/faqs/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Page", key: "page" },
  { title: "Page Display", key: "page_display" },
