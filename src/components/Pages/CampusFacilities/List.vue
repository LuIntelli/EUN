...
import { listCampusFacilities, deleteCampusFacilities } from "@/services/Pages";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listCampusFacilities(); // from your services
...
const handleDelete = async (id) => {
  try {
    await deleteCampusFacilities(id);
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/campus-facilities/edit/${row.id}`),
        },
        row.id
      );
    },
