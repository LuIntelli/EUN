...
import { listUpcomingEvents, deleteUpcomingEvents } from "@/services/Pages";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listUpcomingEvents(); // from your services
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/page/upcoming-events/${row.id}`),
        },
        row.id
      );
    },
