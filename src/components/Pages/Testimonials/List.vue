...
import { listTestimonials, deleteTestimonials } from "@/services/Pages";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listTestimonials(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/page/testimonials/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Student Program", key: "student_program" },
