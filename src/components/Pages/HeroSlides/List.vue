...
import { listHeroSlides, deleteHeroSlide } from "@/services/Pages";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listHeroSlides(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/page/hero-slides/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Home page Title", key: "homepage_title" },
