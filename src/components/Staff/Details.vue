...
import { editStaff, staffDetails, listStaffPosition } from "@/services/Staff";
import {
  listDepartments,
  listFaculties,
...
onMounted(async () => {
  ...
  try {
    ...
    console.log(departmentData.results.data, res.results);
    ...
    console.error("Error loading staff positions", err);
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    if (selected) formData.position_title = selected?.label;
  }
);
watch(
  () => formData.department,
...
const fetchData = async () => {
  try {
    ...
    const data = await staffDetails(id);
    Object.assign(formData, data.data);
    photoPreview2.value = formData["photo"];
...
const handleSubmit = async () => {
  try {
    ...
    await editStaff(route.params.id, formData);
