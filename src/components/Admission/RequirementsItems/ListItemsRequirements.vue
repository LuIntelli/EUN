<template>
  <div class="p-6">
    <h2 class="text-2xl font-bold text-[rgba(61,58,121,1)] mb-4">
      List of Admission Items Requirements
    </h2>

    <n-data-table
      :columns="columns"
      :data="itemsRequirements"
      :loading="loading"
      :bordered="true"
...
import {
  listAdmissionRequirementsItems,
  deleteAdmissionRequirementsItem,
} from "@/services/Admission";

const itemsRequirements = ref([]);
...
const fetchItemsRequirements = async () => {
  loading.value = true;
  try {
    const res = await listAdmissionRequirementsItems(); // from your services
    itemsRequirements.value = res.results.data;
  } catch (err) {
    console.error("Error fetching All the Items-requirements:", err.message);
...
const goToDetails = (id) => {
  router.push(`/admission-requirements-items/${id}`);
};
...
const handleDelete = async (id) => {
  try {
    await deleteAdmissionRequirementsItem(id);
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/admission-requirements-items/${row.id}`),
        },
