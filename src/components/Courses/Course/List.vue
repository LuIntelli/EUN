...
import { listCourse, deleteCourse } from "@/services/Courses";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await listCourse(); // from your services
...
const columns = [
    ...
    render(row) {
      return h(
        "a",
        {
          ...
          onClick: () => router.push(`/courses/edit/${row.id}`),
        },
        row.id
      );
    },
  ...
  { title: "Degree Type", key: "degree_type" },
  { title: "Duration Years", key: "duration_years" },
