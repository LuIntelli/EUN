...
import { editCourse, courseDetails } from "@/services/Courses";
import {
  listDepartments,
  listFaculties,
...
const fetchData = async () => {
  try {
    const res = await courseDetails(route.params.id);
    Object.assign(formData, res);
    photoPreview.value = formData["icon_image"];
    photoPreview1.value = formData["hero_image"];
    photoPreview2.value = formData["curriculum_pdf"];
    delete formData["curriculum_pdf"];
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log("Start submitting...", formData);
    await editCourse(route.params.id, formData);
